...
void IRDaikinESP::begin() { _irsend.begin(); Serial.println("begin fatto");}  ///MODIFICATO

#if SEND_DAIKIN
void IRDaikinESP::send() {
  checksum();
  _irsend.sendDaikin(daikin);
  Serial.println("invio IR eseguito con stringa");  //DeBUG
}
#endif  // SEND_DAIKIN
...
void IRDaikinESP::stateReset() {
  ...
  checksum();
  Serial.println("reset state eseguito");
}
