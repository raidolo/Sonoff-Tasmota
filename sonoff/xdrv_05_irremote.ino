...
IRDaikinESP *daikinir = NULL;
//IRDaikinESP daikinir(pin[GPIO_IRSEND]);
//daikinir.begin();
/*
...
const char kHvacModeOptions[] = "HDCFA";  
#endif
...
#ifdef USE_IR_HVAC
daikinir = new IRDaikinESP(pin[GPIO_IRSEND]);
...
/// FARE FUNZIONI DAIKIN  
boolean IrDaikin(const char *HVAC_Vendor, const char *HVAC_Mode, const char *HVAC_FanMode, boolean HVAC_Power, int HVAC_Temp)
{
    //Initialize the map of supported HVAC
      //daikinir(pin[GPIO_IRSEND]);
      daikinir->begin();
  ...
  if (!error)
  {
      ...
      //HVAC_Mode possible value Hot/Cold/Dry/Fan/Auto
      uint8_t operatingModeCmd;
      switch (toupper(HVAC_Mode[0]))
      {
        case 'H':
          operatingModeCmd = kDaikinHeat;
        break;
        case 'C':
          operatingModeCmd = kDaikinCool;
        break;
        case 'D':
          operatingModeCmd = kDaikinDry;
        break;
        case 'F':
          operatingModeCmd = kDaikinFan;
        break;
        case 'A':
        default:
          operatingModeCmd = kDaikinAuto;
        break;

      }

      //Set the FAN SPEED
      //possible value A12345S
      uint8_t fanSpeedCmd;
      char *p;
      if (HVAC_FanMode == NULL) {
        p = (char *)kFanSpeedOptions; // default FAN_SPEED_AUTO
      ...
      switch (p[0]) {
        ...
        case 'S':
        default:
          fanSpeedCmd = kDaikinFanQuiet;
          break;
      }


      //Set the temperature
      uint8_t temperatureCmd = HVAC_Temp;
      //Set Vertical swing
      uint8_t swingVCmd = false;
      //Set Horizontal swing
      uint8_t swingHCmd = false;




  daikinir->setFan(fanSpeedCmd);
  daikinir->setMode(operatingModeCmd);
  daikinir->setTemp(temperatureCmd);
  daikinir->setSwingVertical(false);
  daikinir->setSwingHorizontal(false); 
//  Serial.println("Daikin state: " + daikinir->toString());
               //snprintf_P(log_data, sizeof(log_data), PSTR("IRHVAC: Received Vendor %s, Power %d, Mode %s, FanSpeed %s, Temp %d"),
               //HVAC_Vendor, HVAC_Power, operatingModeCmd, fanSpeedCmd, HVAC_Temp);
               //AddLog(LOG_LEVEL_DEBUG);
               //snprintf_P(log_data, sizeof(log_data), PSTR("Daikin setup Vendor %s, Power %d, Mode %s, FanSpeed %s, Temp %d"),
               //HVAC_Vendor, HVAC_Power, HVAC_Mode, fanSpeedCmd, temperatureCmd);
               //AddLog(LOG_LEVEL_DEBUG);
#if SEND_DAIKIN
 daikinir->send();
#endif 
    ...
    if (XdrvMailbox.data_len) {
      ...
      if (!root.success()) {
        ...
        HVAC_Temp = root[D_JSON_IRHVAC_TEMP];
               ...
               //AddLog(LOG_LEVEL_DEBUG);
      if (HVAC_Vendor != NULL){
          error = IrDaikin(HVAC_Vendor, HVAC_Mode, HVAC_FanMode, HVAC_Power, HVAC_Temp);
        }
/*
